...
### Инфраструктура из v0.64

Следующие компоненты создаются в [md-transcript-import](md-transcript-import.md) (v0.64) и переиспользуются:

- `SpeakerInfo` модель в `schemas.py` — named_speakers, anonymous_speakers, scenario
- `speaker_utils.py` — парсинг спикеров из текста MD
- `VideoMetadata.speaker_info` — результат парсинга, доступна во всех stages
- Clean prompt — базовое сохранение меток спикеров

### Data flow (v0.65): speaker_info → промпты → saver

В v0.64 `speaker_info` используется только для UI. В v0.65 добавляется:

```
1. Clean/Longread/Summary stages (v0.65):
   speaker_info = context.get_result("parse").speaker_info
   → формирует блок контекста для user prompt:
     "Тип: линейка\nСпикеры: ...\nВедущий: ..."

2. Saver (_save_chunks_json) (v0.65):
...
### Этап 1: Контекст спикеров в промптах (v0.65)

> `SpeakerInfo`, `speaker_utils.py`, `VideoMetadata.speaker_info` и базовый clean — созданы в v0.64.

- [ ] Формировать блок контекста мультиспикерности для user prompt из `speaker_info`
- [ ] Интегрировать контекст в Clean/Longread/Summary/Story stages

### Этап 2: Адаптация промптов (v0.65)

Стратегия: условные секции в существующих файлах (не отдельные файлы).

- [ ] Добавить секцию "Мультиспикерный контент" в `cleaning/system.md`
