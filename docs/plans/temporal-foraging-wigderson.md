# Plan: Создание требования для проекта md2gdoc

## Context

Пользователь вручную конвертирует MD-лонгриды в Google Docs (через Obsidian → копирование → вставка).
Нужен сервис с admin-панелью, мониторингом папок (локальных + Google Drive) и автоконвертацией.

Референсный проект (n8n workflow + Python скрипты) есть в `docs/reference/`, но его архитектура
была ограничена n8n и требовала платного CloudConvert.

## Действие

Создать файл требования:
`docs/reference/Obsidian → Google Docs/REQUIREMENT-md2gdoc-service.md`

Содержание ниже — готовый документ для записи.
...
- [ ] MD → HTML конвертер (markdown-it или Python-Markdown + расширения)
