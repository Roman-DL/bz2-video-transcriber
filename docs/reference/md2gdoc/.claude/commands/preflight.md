...
Прочитай модульные документы `docs/architecture/` для затрагиваемых подсистем:
- `01-converter.md` — конвертация MD↔HTML↔GDoc
- `02-rules-engine.md` — правила, Sync Manager, SQLite
- `03-google-drive.md` — Google Drive API client
...
Определи какие модули будут затронуты задачей, затем проверь:
...
> Это предварительный список. После реализации `/finalize` проверит фактические изменения.
