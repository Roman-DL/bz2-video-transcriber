...
|-------------|------|--------|
| Описание проекта | CLAUDE.md | "MD → Google Docs конвертация" |
| Команды запуска | CLAUDE.md | Секция "Quick Start" |
| Workflow разработки | CLAUDE.md | /preflight → реализация → /finalize |
| Навигация по docs/ | CLAUDE.md | Таблицы документации |
| Общие ограничения | CLAUDE.md | "НИКОГДА не хардкодить промпты" |
| **Правила модуля** | **Существующий rule** | "Каллауты — HTML таблица 2×1" → `converter.md` |
| **Правила нового модуля** | **Новый rule** | Правила для sync → `sync-manager.md` с globs |
...
```markdown
---
globs: backend/app/services/module/**
---
...
- 2-3 критичных ограничения (про polling, про Google API)
