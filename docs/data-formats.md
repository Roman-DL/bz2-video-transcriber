...
| Тип | Код мероприятия | `ПШ`, `Форум TABTeam` | ✓ |
| Поток | Код аудитории | `SV`, `НП`, `Бизнес` | ✗ |
| Разделитель | Точка + пробел после типа/потока | `.` | ✓ |
| Тема | Свободный текст | `Группа поддержки`, `История` | ✓ |
| Спикер | В скобках | `Светлана Дмитрук` | ✓ |

### Regex для парсинга

```python
EVENT_PATTERN = r'^(\d{4}\.\d{2}\.\d{2})\s+(.+?)\.\s+(.+?)\s+\(([^)]+)\)(?:\.\w+)?$'

# Группы захвата:
# 1: date          → 2025.04.07
# 2: event_group   → ПШ.SV (тип + опциональный поток)
# 3: title         → Группа поддержки
# 4: speaker       → Светлана Дмитрук
```
...
└── 2025/
    ├── ПШ/
    │   ├── 04.07 SV. Группа поддержки (Светлана Дмитрук)/
    │   │   ├── 2025.04.07 ПШ.SV. Группа поддержки (Светлана Дмитрук).mp4
    │   │   ├── audio.mp3
    │   │   ├── transcript_chunks.json
    ...
    │   │   └── pipeline_results.json
    │   │
    │   └── 04.07 SV. История (Антоновы Дмитрий и Юлия)/
    │       └── ...  (— история.md вместо — лонгрид.md)
    │
    └── 05 Форум TABTeam/
        └── История (Иванов Дмитрий)/
            └── ...
```
...
### 1. transcript_chunks.json (BZ2-Bot v1.0, v0.60+)

**Назначение:** Chunks для импорта в БЗ 2.0 через BZ2-Bot админ-портал.

**Формат:** [BZ2-Bot chunk-import контракт v1.0](requirements/chunk-import-contract.md). Файл готов для загрузки без трансформации.

**Использование в БЗ 2.0:**
- Загружается через админ-портал BZ2-Bot
- Каждый chunk индексируется отдельно с embeddings
- RAG Search находит релевантные chunks по вопросу пользователя
- `description` используется как семантический индекс для поиска
- `short_description` — для Telegram-рассылки

```json
{
  "version": "1.0",
  "materials": [{
    "description": "Семантический индекс: подготовка и проведение Группы поддержки, формат встречи, вовлечение участников, чек-лист организатора, практические инструменты ведущего.",
    "short_description": "Светлана Дмитрук — как организовать и провести Группу поддержки: подготовка помещения, структура встречи, техники вовлечения.",
    "metadata": {
      "video_id": "2025-04-07_ПШ-SV",
    ...
    "chunks": [
      {
        "text": "Тема: Подготовка и проведение Группы поддержки\nСпикер: Светлана Дмитрук | ПШ.SV | 07.04.2025\n\n## Введение и цели\n\nСегодня мы поговорим о том, как правильно готовить и проводить Группу поддержки...",
        "metadata": {
          "chunk_id": "2025-04-07_ПШ-SV_001",
          "chunk_index": 1,
      ...
      {
        "text": "Тема: Подготовка и проведение Группы поддержки\nСпикер: Светлана Дмитрук | ПШ.SV | 07.04.2025\n\n## Подготовка помещения (1/2)\n\nПеред проведением Группы поддержки важно подготовить помещение...",
        "metadata": {
          "chunk_id": "2025-04-07_ПШ-SV_002",
          "chunk_index": 2,
          "topic": "Подготовка помещения",
          "word_count": 420
        }
      }
    ]
...
| Поле | Тип | Описание |
|------|-----|----------|
| `version` | string | Версия контракта (`"1.0"`) |
| `materials` | array | Массив материалов (всегда 1 элемент) |

**Material:**

| Поле | Тип | Описание |
|------|-----|----------|
| `description` | string | Семантический индекс для embedding-поиска (2-5 предложений, Claude) |
| `short_description` | string | Краткое описание для Telegram (1-2 предложения, Claude) |
| `metadata` | object | Метаданные видео |
| `chunks` | array | Массив чанков с контекстной шапкой |

**Chunk:**

| Поле | Тип | Описание |
|------|-----|----------|
| `text` | string | Контекстная шапка + H2 заголовок + текст (до 600 слов) |
| `metadata.chunk_id` | string | ID: `{video_id}_{index:03d}` |
| `metadata.chunk_index` | int | Порядковый номер (1, 2, 3...) |
| `metadata.topic` | string | Тема блока (оригинальный H2, без суффикса) |
| `metadata.word_count` | int | Количество слов |

### Контекстная шапка

Каждый chunk содержит шапку для standalone-понимания при RAG-поиске:
...
Если H2-секция превышает 600 слов, она разбивается по параграфам. При этом:
- `metadata.topic` — оригинальный заголовок без суффикса
...
| | transcript_chunks.json | pipeline_results.json |
|---|---|---|
| **Назначение** | Импорт в BZ2-Bot | Просмотр в UI |
| **Формат ключей** | snake_case (BZ2-Bot контракт) | camelCase (ADR-013) |
| **Структура** | `materials[].chunks[]` с контекстной шапкой | `chunks.chunks[]` (raw data) |
| **Description** | Есть (Claude-generated) | Нет |
...
**Назначение:** Типы мероприятий, их категории и потоки (v0.69+)

```yaml
# Единый формат: {дата} {тип}[.{поток}]. {тема} ({спикер}).ext
# category: regular | offsite
