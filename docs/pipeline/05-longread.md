...
> **v0.67+:** Авто-выбор single-pass (1 LLM вызов) или map-reduce по размеру контекста модели.
...
## Архитектура: Авто-выбор пути (v0.67+)

`LongreadGenerator` автоматически выбирает путь генерации по размеру контекста модели:
...
CleanedTranscript
       ...
       ▼
┌─────────────────────────────┐
│   _fits_in_context()?       │  → Оценка: Russian ~2.5 tokens/char
│   overhead ~45K tokens      │    (system prompt + output reserve)
└────────────┬────────────────┘
             │
      ┌──────┴───────┐
      │              │
...
### Single-pass (v0.67+) — default для Claude

Когда текст помещается в контекст модели (Claude 200K → ~62K символов):

- **1 LLM вызов** — весь транскрипт + промпт → полный лонгрид (JSON)
- Промпты: `system.md` + `instructions.md` + `template.md`
- `SINGLE_PASS_MAX_TOKENS = 16384` — передаётся как `num_predict`
- Результат: полный JSON с секциями, intro, conclusion, классификацией

### Map-Reduce — fallback для моделей с малым контекстом

Когда текст НЕ помещается в контекст (Ollama 8-32K):

```
SPLIT → OUTLINE (optional) → MAP (sections) → REDUCE (frame) → Longread
```
...
Оба пути используют одни и те же файлы промптов (`config/prompts/longread/`). Single-pass отправляет все три файла в одном запросе, map-reduce использует их поэтапно.
...
| `template.md` | Шаблон JSON-ответа |

Архитектура промптов (v0.67+):
```
Single-pass: system.md + instructions.md + template.md → Полный лонгрид
Map-reduce:  system.md + instructions.md → Section generation
