...
|----------|----------|----------|
| `CLEANER_MODEL` | claude-sonnet-4-6 | Модель для очистки (v0.29+, env variable) |
...
По умолчанию используется `claude-sonnet-4-6` (v0.29+). Исторически тестировались локальные модели:
