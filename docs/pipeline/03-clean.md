...
|----------|--------|---------|
| Слова-паразиты | "ну", "вот", "как бы", "эээ" | Задача 1: LLM удаляет |
| Отвлечения | "видно экран?", "вы меня слышите?" | Задача 1: LLM удаляет |
| Ошибки Whisper | "Формула один", "херболайф" | Задача 2: LLM исправляет по глоссарию |
| Термины Herbalife | "гербалайф", "СВ", "гет тим" | Задача 2: LLM нормализует по контексту |
| Имена людей | "Марк Грюз", "Стэпен Грицани" | Задача 2: LLM исправляет по разделу people |
| Техшум | проверка микрофона, счёт, рассадка | Задача 3: LLM удаляет |
| Повторы/заикания | "нам нужно, нам нужно", "сейчас, сейчас" | Задача 3: LLM сокращает |
| Обрывки фраз | "я думаю что мы... я считаю что" | Задача 3: LLM удаляет оборванную часть |
...
│   (1-2 чанка)   │  (для записей >40K chars)
└────────┬────────┘
         │
         ▼
┌─────────────────┐
...
│ (claude-haiku)  │  3 задачи: паразиты + термины + причёсывание
└────────┬────────┘
         │
         ▼
┌─────────────────┐
...
└────────┬────────┘
         │
         ▼
  CleanedTranscript (ожидание: ~55KB, ~20% reduction)
```
...
| `CHUNK_OVERLAP_CHARS` | 3000 | Перекрытие между частями (v0.65+) |
...
|-------------|------------|------------|
| Опытный | 5-15% | Подготовленное выступление |
| Средний | 15-20% | Вебинар, живое общение |
| Неопытный | 20-25% | Много запинок, техпроблемы, повторы |

**Важно:** С задачей 3 (причёсывание) ожидается +5-10% дополнительной редукции. Сокращение >30% означает возможную потерю контента. Сокращение >40% — скорее всего, LLM сделал саммари вместо очистки.
