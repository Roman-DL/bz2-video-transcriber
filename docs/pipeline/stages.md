...
├── parse_stage.py           # Парсинг имени файла + MD enrichment (v0.84+)
├── transcribe_stage.py      # Транскрипция через Whisper
├── clean_stage.py           # Очистка транскрипта
├── slides_stage.py          # Извлечение текста со слайдов (v0.84+, optional)
...
| `slides` | `["clean"]` | `slides_input` (metadata) | `SlidesExtractionResult` |
...
| `save` | `["parse", "transcribe", "clean", "chunk"]` | All results | `SaveResult` |
...
class TelegramSummaryStage(BaseStage):
    ...
    async def execute(self, context: StageContext) -> TelegramSummary:
        ...
        model = context.get_metadata("model_overrides", {}).get("telegram_summary")
        ...
        async with self.processing_strategy.create_client(actual_model) as ai_client:
            prompt = f"Создай превью для Telegram (150 символов). Заголовок: {longread.title}"
            response, usage = await ai_client.generate(prompt=prompt, model=actual_model)
