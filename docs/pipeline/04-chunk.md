...
### Преамбула и заключение (v0.74+)

Текст **до** первого `## ` (преамбула) и текст **после** `---` (заключение) не чанкируются — остаются в лонгриде для читателей. В чанки попадают только тематические секции с H2 заголовками.

- **Преамбула:** YAML frontmatter, H1 заголовок, вступительный текст (introduction)
- **Заключение:** итоговый абзац (conclusion)
- **RAG-покрытие:** обзорная информация дублируется в `description` из `TranscriptChunks`
