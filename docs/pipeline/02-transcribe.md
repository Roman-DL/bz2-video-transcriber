...
| **Output** | `tuple[RawTranscript, Path \| None]` | Транскрипт и путь к аудиофайлу (None для MD) |

**Зависимости:** `depends_on = ["parse"]`

## MD-транскрипты (v0.64+)

Для `.md` файлов (готовые транскрипты из MacWhisper) Whisper пропускается:

```
MD файл (.md)
       │
       ▼
Чтение текста (Path.read_text)
       │
       ▼
RawTranscript (один сегмент, audio_path=None)
```

- **Маркер источника:** `whisper_model="macwhisper-large-v2"`
- **Длительность:** оценка по количеству слов (~130 слов/мин для русской речи)
- **Спикеры:** `speaker_utils.parse_speakers()` парсит спикеров из текста → `SpeakerInfo`
- **Проверка:** `is_transcript_file()` из `app.utils`

## Архитектура (медиафайлы)

Транскрипция медиафайлов выполняется в два этапа для надёжности:
...
2. `Path | None` — путь к извлечённому аудиофайлу (для сохранения в архив), `None` для MD-файлов
