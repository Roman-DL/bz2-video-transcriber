...
При прямой транскрипции длинных видео (55+ мин) Whisper API давал артефакты — повторяющиеся фразы. Извлечение аудио через ffmpeg решает эту проблему:

- **Надёжность** — аудиоформаты стабильно обрабатываются Whisper
- **Размер** — MP3 ~10% от размера видео (быстрее передаётся)
