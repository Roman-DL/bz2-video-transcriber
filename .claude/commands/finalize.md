...
> **Параметр (опционально):** путь к спецификации, требованию или плану — `/finalize docs/superpowers/specs/...`
...
Если передан параметр — прочитай указанный документ (спецификация, требование или план) для контекста: что планировалось, какие решения принимались. Это помогает точнее оценить расхождения между планом и фактом.

---

## Шаг 1: Понять что изменилось

Если передан параметр (путь к документу) — прочитай его первым делом. Это даст контекст: что планировалось, какие были решения.
...
- Если есть спецификация/план — что расходится с фактической реализацией?
...
- Документ из параметра (если передан — спецификация, требование или план)
...
### Housekeeping: планы, спеки, требования

После завершения фичи — разная судьба у разных типов документов:

| Тип | Действие | Почему |
|-----|----------|--------|
| **Планы** (`docs/plans/`, `docs/superpowers/plans/`) | **Переместить** в `docs/plans/archive/` | Рабочие чеклисты; `docs/plans/` — основное место для планов из requirements |
| **Спецификации** (`docs/superpowers/specs/`) | **Архивировать** → `docs/superpowers/specs/archive/` | Содержат дизайн-решения, могут понадобиться для контекста |
| **Требования** (`docs/requirements/`) | **Архивировать** → `docs/requirements/archive/` | Содержат бизнес-контекст и критерии приёмки |

Порядок действий:
1. **Планы** — переместить в `docs/plans/archive/` (из `docs/superpowers/plans/` — тоже сюда)
2. **Спеки** — обновить `**Статус:**` на `Done (vX.Y.Z)`, переместить в `archive/`
3. **Требования** — обновить `**Статус:**` на `Done (vX.Y.Z)`, переместить в `archive/`

> Всегда спрашивать подтверждение перед удалением/перемещением.
...
5. После успешного `cz bump` → обнови строку версии в секции «Текущий статус» CLAUDE.md (номер версии и диапазон в ссылке на CHANGELOG)
...
**Планы:** {архивировать X → plans/archive/ / нет завершённых}
**Спеки:** {архивировать X → specs/archive/ (Done vN.N) / нет завершённых}
**Требования:** {архивировать X → requirements/archive/ (Done vN.N) / нет завершённых}
```
