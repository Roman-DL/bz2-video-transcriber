...
if __name__ == "__main__":
    ...
    async def run_tests():
        ...
        try:
            system_prompt = load_prompt("longread", "system", settings)
            instructions = load_prompt("longread", "instructions", settings)
            template = load_prompt("longread", "template", settings)
        ...
        # Test 4: Context fitting logic
        print("\nTest 4: Context fitting logic...", end=" ")
        try:
            generator = LongreadGenerator(None, settings)  # type: ignore

            # Claude 200K context: should fit 43K chars
            generator.context_tokens = 200000
            text_43k = "А" * 43000
            assert generator._fits_in_context(text_43k), "43K chars should fit in 200K context"

            # Small model 8K context: should NOT fit 43K chars
            generator.context_tokens = 8192
            assert not generator._fits_in_context(text_43k), "43K chars should not fit in 8K context"

            # Small model with small text
            text_1k = "А" * 1000
            generator.context_tokens = 32768
            assert generator._fits_in_context(text_1k), "1K chars should fit in 32K context"
